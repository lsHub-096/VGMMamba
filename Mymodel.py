...
class VGM(nn.Module):
    def __init__(self, n_fea_middle, n_fea_in=5, n_fea_out=3):
    ...
    def forward(self, img):
        ...
        x_mean = torch.mean(img, dim=1, keepdim=True)[0]
        input = torch.cat([img, x_max, x_mean], dim=1)  # 对应
        x_1 = self.conv1(input)
        first_feature = self.depth_conv(x_1)
        VGM_feature = self.conv2(first_feature)
        return VGM_feature
...
class FeedForward(nn.Module):
    def __init__(self, dim, expand, bias):
        ...
        self.dwconv3 = nn.Conv2d(hidden_features, hidden_features * 2, kernel_size=1)
        self.expand_illum = nn.Conv2d(hidden_features, hidden_features, kernel_size=1)
        self.illum_attention = AttentionModule(in_channels=hidden_features)
    ...
    def forward(self, x_in, vgm_feature):#illum:[1,96,256,256] x_in[1,48,256,256]
        x = self.project_in(x_in)#[1,96,256,256]
        hidden_features = x.shape[1]
        attn1 = self.dwconv(x)#[1, 64, 256,256]
        attn2 = self.dwconv2(attn1)#[1,64,256,256]
        # 将 illum 扩展成与 x 相同通道数
        if vgm_feature.shape[1] != hidden_features:
            illum_expanded = F.interpolate(vgm_feature, size=x.shape[2:], mode='bilinear', align_corners=False)
            illum_expanded = self.expand_illum(illum_expanded)  # 假设你加了一个 Conv1x1 来扩展通道
        else:
            illum_expanded = vgm_feature

        # 应用注意力机制
        illum_attn = self.illum_attention(illum_expanded)
        illum_attn = self.dwconv3(illum_attn)

        # 分成两个分支
        illum1, illum2 = illum_attn.chunk(2, dim=1)
...
class VGMMamba(nn.Module):
    def __init__(self, nf=48,
                 ...
                 num_layer=3):
        super(VGMMamba, self).__init__()

        self.nf = nf
        self.depths = depths

        self.vgm = VGM(embed_dim)

        self.conv_first_1 = nn.Conv2d(3, nf, 3, 1, 1, bias=False)
        self.conv_first_1_fea = nn.Conv2d(6, int(nf * mlp_ratio), 3, 1, 1)
        ...
        self.conv_first_2_fea = nn.Conv2d(6, int(nf * 2 * mlp_ratio), 3, 1, 1)
        ...
        self.conv_first_3_fea = nn.Conv2d(6, int(nf * 4 * mlp_ratio), 3, 1, 1)
        ...
        self.conv_first_4_fea = nn.Conv2d(6, int(nf * 4 * mlp_ratio), 3, 1, 1)
        ...
        self.conv_first_5_fea = nn.Conv2d(6, int(nf * 2 * mlp_ratio), 3, 1, 1)
        ...
        self.conv_first_6_fea = nn.Conv2d(6, int(nf * mlp_ratio), 3, 1, 1)
    ...
    def forward(self, x_in):
        ...
        x_in_cat = torch.cat((x_in, x_vgm), dim=1)
